...
class fm_model {
	...
	public:
		...
		fm_model();
        fm_model(unsigned in_num_attribute, double in_init_stdev, const vector<int> &dim);
		void debug();
		void init();
...
fm_model::fm_model( unsigned in_num_attribute, double in_init_stdev, const vector<int> &dim )
    :num_attribute(in_num_attribute), init_stdev(in_init_stdev), k0(dim[0] != 0), k1(dim[1] != 0), num_factor(dim[2])
{
    init();
}
